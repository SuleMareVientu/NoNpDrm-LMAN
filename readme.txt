...
- Imported games behave as purchased games and allow the use of game updates seamlessly downloaded from the Sony Interactive Entertainment Network (PlayStation Network) as long as these updates run on firmware 3.60 and lower.
...
## Installation
Download the latest official version [nonpdrm.skprx](https://github.com/TheOfficialFloW/NoNpDrm/releases), copy it to `ux0:tai` and modify the `ux0:tai/config.txt` file to add the path to the module under `*KERNEL` as follows
Download the latest unofficial\mod version [nonpdrm.skprx](https://sites.google.com/site/theleecherman), copy it to `ux0:tai` and modify the `ux0:tai/config.txt` file to add the path to the module under `*KERNEL` as follows
...
│       └───sce_sys
│           └───package
│               └───work.bin (copied or overwritten from ux0:nonpdrm/license/theme/THEME_ID/6488b73b912a753a492e2714e9b38bc7.rif)
```
...
### v1.4 Unofficial
- Resolved an issue regarding licenses generation for shared DLCs by Fumu-no-Kagomeko.

### v1.3 Unofficial
- Added support to generate fake license files for themes.
- Added support to check the existence of fake license files before generating it.
...
- Thanks to TheFloW for this plugin.